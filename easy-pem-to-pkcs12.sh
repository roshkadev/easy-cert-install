...
if [[ -z "$PEM_KEY" || -z "$PEM_CERT" || -z "$PKCS12_OUT" || -z "$ALIAS" ]]; then
    echo "Usage: $MY_SCRIPT PEM_KEY_FILE.key PEM_CERTFICATE_FILE.crt PKCS12_OUT_FILENAME.jks ALIAS\nWhere parameter are mostly self explanatory. I guess... ALIAS is the name to use to store values in PKCS12"
    exit -1
fi
...
openssl pkcs12 -export -in $PEM_CERT -inkey $PEM_KEY -out $PKCS12_OUT -name $ALIAS
